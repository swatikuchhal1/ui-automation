...
export class Rules extends BasePage {
    ...
    async login(username, password) {
        ...
        const img1 = PNG.sync.read(fs.readFileSync('../ui-automation/screenshots/1_Login.png'))
        const img2 = PNG.sync.read(fs.readFileSync('../ui-automation/masterscreenshots/1_Master_Login.png'))

        const { width, height } = img1
        const diff = new PNG({ width, height })
        if (imageMatch == 'true') {
            pixelmatch(img1.data, img2.data, diff.data, width, height, { threshold: 0.1 })
            fs.writeFileSync('../ui-automation/screenshots/1_Login_diff.png', PNG.sync.write(diff))
        }
    ...
    async gotoCXMPage() {
        ...
        const img1 = PNG.sync.read(fs.readFileSync('../ui-automation/screenshots/2_HomePage.png'))
        const img2 = PNG.sync.read(fs.readFileSync('../ui-automation/masterscreenshots/2_Master_HomePage.png'))
        const { width, height } = img1
        const diff = new PNG({ width, height })
        if (imageMatch == 'true') {
            pixelmatch(img1.data, img2.data, diff.data, width, height, { threshold: 0.1 })
        ...
        const img4 = PNG.sync.read(fs.readFileSync('../ui-automation/masterscreenshots/3_Master_CXM.png'))
